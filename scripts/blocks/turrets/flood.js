...
Flood.buildType = () => {
	const ent = extendContent(LiquidTurret.LiquidTurretBuild, Flood, {
        ...
        updateTile(){
            this.unit.ammo = (this.getLiquidsAmounts().size > 0 ? (this.getLiquidsAmounts().size == 2 ? (Math.max(this.getLiquidsAmounts().get(0), this.getLiquidsAmounts().get(1))) : this.getLiquidsAmounts().get(0)) : 0.0);

            this.super$updateTile();
        }, 
		
        sense(sensor){
        	var ammo = this.getLiquidsAmounts().size > 0 ? (this.getLiquidsAmounts().size == 2 ? (Math.max(this.getLiquidsAmounts().get(0), this.getLiquidsAmounts().get(1))) : this.getLiquidsAmounts().get(0)) : 0.0;
            switch(sensor){
                case LAccess.ammo: return ammo;
