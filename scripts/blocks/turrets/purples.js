...
const ReflectionLuxFragBullet = extend(BasicBulletType, {
    draw(b) {
        Draw.color(C.magicLight);
        Fill.circle(b.x, b.y, 2.4); 
    }
});
ReflectionLuxFragBullet.damage = 220;
ReflectionLuxFragBullet.speed = 12.5;
ReflectionLuxFragBullet.lifetime = 15;
ReflectionLuxFragBullet.hitEffect = E.magicBulletHitSmall;
ReflectionLuxFragBullet.despawnEffect = E.magicBulletHitSmall;
ReflectionLuxFragBullet.hitColor = C.magicLight.cpy();
ReflectionLuxFragBullet.shootEffect = E.magicShootEffect;

const ReflectionLuxBullet = extend(BasicBulletType, {
    draw(b) {
        Draw.color(C.magicLight, C.magicDark, b.fin());
        Fill.circle(b.x, b.y, 2.8 + b.fslope()); 
    }, 
    ...
    hit(b, x, y) {
        ...
        for(var i = 0; i < 2; i++){
	        ...
	        Call.createBullet(ReflectionLuxFragBullet, b.team, b.x + v.x, b.y + v.y, v.angle() - 180, 60, 1.0, 1.0);
	     } 
    } 
});
ReflectionLuxBullet.damage = 180;
ReflectionLuxBullet.speed = 5.5;
ReflectionLuxBullet.lifetime *= 1.2;
ReflectionLuxBullet.reloadMultiplier = 0.75;
ReflectionLuxBullet.ammoMultiplier = 2.0;
ReflectionLuxBullet.hitEffect = E.magicBulletHit;
ReflectionLuxBullet.despawnEffect = E.magicBulletHit;
ReflectionLuxBullet.hitColor = C.magicLight.cpy();
ReflectionLuxBullet.shootEffect = E.magicShootEffectBig;
ReflectionLuxBullet.ammoMultiplier = 3.0;
...
Reflection.ammo(
    ...
    F.fi("lux"), ReflectionLuxBullet,
    F.fi("laculis"), ReflectionLaculisBullet
);
Reflection.requirements = ItemStack.with(F.fi("amethyst"), 610, F.fi("palladium"), 400, Items.silicon, 525, Items.surgeAlloy, 360, F.fi("orbon"), 180, F.fi("cutol"), 285);
...
Decomposer.requirements = ItemStack.with(F.fi("amethyst"), 1250, F.fi("meteorite"), 800, Items.silicon, 750, Items.thorium, 925, F.fi("contritum"), 450, F.fi("laculis"), 650);
...
F.techNode(Blocks.duo, Mirror, ItemStack.with(F.fi("amethyst"), 6000, Items.graphite, 3500, Items.lead, 8000));
F.techNode(Mirror, Recours, ItemStack.with(F.fi("amethyst"), 20000, F.fi("cutol"), 7500, Items.silicon, 14000, Items.plastanium, 12000, Items.thorium, 16000));
F.techNode(Recours, Reflection, ItemStack.with(F.fi("amethyst"), 45000, F.fi("palladium"), 28000, Items.silicon, 30000, Items.surgeAlloy, 18000, F.fi("orbon"), 10000, F.fi("cutol"), 15000));
F.techNode(Reflection, Decomposer, ItemStack.with(F.fi("amethyst"), 90000, F.fi("meteorite"), 45000, Items.silicon, 40000, Items.thorium, 55000, F.fi("contritum"), 25000, F.fi("laculis"), 35000));
