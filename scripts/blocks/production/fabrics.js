...
CutolCrafter.updateEffect = Fx.smeltsmoke;
...
CutolCrafter.idleSound = Sounds.respawning;
CutolCrafter.idleSoundVolume = 0.2;
...
LargePhaseWeaver.updateEffect = Fx.smeltsmoke;
LargePhaseWeaver.consumes.power(12);
LargePhaseWeaver.consumes.liquid(Liquids.cryofluid, 0.04);
LargePhaseWeaver.consumes.items(new ItemStack(Items.thorium, 7), new ItemStack(Items.sand, 12));
LargePhaseWeaver.requirements = ItemStack.with(Items.phaseFabric, 120, F.fi("titanium-plate"), 105, Items.silicon, 90, Items.surgeAlloy, 75);
LargePhaseWeaver.outputItem = new ItemStack(Items.phaseFabric, 6);
LargePhaseWeaver.idleSound = Sounds.respawning;
LargePhaseWeaver.idleSoundVolume = 0.05;
...
Diluent.requirements = ItemStack.with(F.fi("palladium"), 75, F.fi("thorium-plate"), 90, Items.silicon, 60);
Diluent.outputLiquid = new LiquidStack(F.fl("helium-liquid"), 15);
Diluent.idleSound = Sounds.respawning;
Diluent.idleSoundVolume = 0.05;
...
const EnergySealer = extendContent(GenericCrafter, "energy-sealer", {
	draw(){
		...
		Draw.rect(Core.atlas.find(this.name + "-anim"), this.x, this.y);
		Draw.mixcol();
	}
});
...
EnergySealer.idleSound = Sounds.respawning;
EnergySealer.idleSoundVolume = 0.05;
EnergySealer.buildCostMultiplier = 0.7;
EnergySealer.itemCapacity = 10;
EnergySealer.category = Category.crafting;
EnergySealer.requirements = ItemStack.with(F.fi("palladium"), 120, F.fi("titanium-plate"), 150, Items.silicon, 75, Items.surgeAlloy, 90, Items.plastanium, 50);

const Crystallizer = extendContent(GenericCrafter, "crystallizer", {});
Crystallizer.buildVisibility = BuildVisibility.shown;
Crystallizer.hasPower = true;
Crystallizer.hasItems = true;
Crystallizer.health = 1260;
Crystallizer.size = 5;
Crystallizer.craftTime = 75;
Crystallizer.craftEffect = E.luxCraft;
Crystallizer.consumes.power(20);
Crystallizer.consumes.items(new ItemStack(F.fi("ruby"), 1), new ItemStack(F.fi("sapphire"), 1), new ItemStack(F.fi("amethyst"), 1), new ItemStack(F.fi("emerald"), 1), new ItemStack(F.fi("diamond"), 1), new ItemStack(F.fi("topaz"), 1));
Crystallizer.requirements = ItemStack.with(F.fi("ruby"), 130, F.fi("diamond"), 85, Items.graphite, 125, Items.surgeAlloy, 75, Items.plastanium, 60);
Crystallizer.outputItem = new ItemStack(F.fi("lux"), 1);
Crystallizer.idleSound = Sounds.respawning;
Crystallizer.idleSoundVolume = 0.05;
Crystallizer.buildCostMultiplier = 0.7;
Crystallizer.itemCapacity = 5;
...
OrbonCrafter.updateEffect = Fx.smeltsmoke;
...
OrbonCrafter.idleSound = Sounds.respawning;
OrbonCrafter.idleSoundVolume = 0.3;
...
const contritumPower = 1500.0/60.0;
...
ContritumCrafter.updateEffect = Fx.smeltsmoke;
...
ContritumCrafter.idleSound = Sounds.respawning;
ContritumCrafter.idleSoundVolume = 0.4;
...
F.techNode(Blocks.phaseWeaver, LargePhaseWeaver, ItemStack.with(Items.phaseFabric, 4000, F.fi("titanium-plate"), 4000, Items.silicon, 8500, Items.surgeAlloy, 5500));
F.techNode(F.fi("lux"), Crystallizer, ItemStack.with(F.fi("ruby"), 20000, F.fi("diamond"), 20000, Items.graphite, 32000, Items.surgeAlloy, 12000, Items.plastanium, 13500));
F.techNode(F.fi("laculis"), EnergySealer, ItemStack.with(F.fi("palladium"), 15000, F.fi("titanium-plate"), 9000, Items.silicon, 22000, Items.surgeAlloy, 7500, Items.plastanium, 13500));
F.techNode(F.fl("helium-liquid"), Diluent, ItemStack.with(F.fi("palladium"), 7500, F.fi("thorium-plate"), 9000, Items.silicon, 14500));
